    ...
    useEffect(() => {
        ...
        const fetchLocation = async () => {
            try {
                ...
                if (
                    data.data.liveLocation?.lat &&
                    data.data.liveLocation?.lng &&
                    data.data.liveLocation?.updatedAt
                ) {
                    setLiveLocation({
                ...
                if (data.data.status && data.data.status !== order.status) {
                    setOrder((prev) =>
        ...
        <div className="min-h-screen mb-20 bg-app-cream">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <span
                        className={`px-4 py-1.5 text-sm font-semibold rounded-full ${order.status === "Delivered" ? "bg-green-100 text-gray-700" : "bg-app-orange/10 text-app-orange"}`}
                    >
                        {order!.status}
                    </span>
                </div>
